...
set -e
# ------------------------------------------------------------------
# SCRIPT 06: RIBOSOMAL TAXONOMIC ASSIGNMENT (BLASTn - PR2 + SILVA)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 1. BLAST against PR2 (Reference for Eukaryotes / Chloroplasts)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 2. BLAST against SILVA SSU (16S/18S Exhaustive)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 4. DATA CLEANING & FORMATTING (Top Hit & Threshold Filters)
# ------------------------------------------------------------------
...
for FILE in "${FILES[@]}"; do
    INPUT="$OUT_DIR/$FILE"
    OUTPUT="$OUT_DIR/${FILE/.tsv/_CLEAN.tsv}"
    
    if [ -s "$INPUT" ]; then 
        if [[ "$FILE" == *"PR2"* ]]; then
            # Specific formatting for PR2 (Splitting the | delimiter)
            echo "  -> Formatting PR2 data for $FILE..."
            ...
            # Standard formatting for SILVA (Taxonomy in column 7)
            echo "  -> Formatting SILVA data for $FILE..."
            awk -F'\t' -v OFS='\t' -v id="$MIN_ID" -v len="$MIN_LEN" '
        ...
        echo "  ⚠️ File $FILE is empty or missing, step skipped."
    fi
done
...
echo "===================================================================="
echo "✅ BLAST ANNOTATION AND FORMATTING COMPLETED!"
echo "📊 Clean results are available in: $OUT_DIR/"
echo "===================================================================="